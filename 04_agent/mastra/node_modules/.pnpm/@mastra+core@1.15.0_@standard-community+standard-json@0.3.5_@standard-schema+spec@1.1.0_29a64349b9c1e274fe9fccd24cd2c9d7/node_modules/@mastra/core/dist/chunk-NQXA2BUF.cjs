...
function normalizeNullishInput(schema, input) {
  ...
  const jsonSchema = chunkVWJ7H3MJ_cjs.standardSchemaToJSONSchema(schema, { io: "input" });
...
function validateToolInput(schema, input, toolId) {
  ...
  const failingNullPaths = new Set(
    validation.issues.filter((issue) => issue.message?.includes("null")).map((issue) => issue.path?.map((p) => typeof p === "object" && "key" in p ? String(p.key) : String(p)).join(".")).filter((p) => !!p)
  );
...
var Tool = class {
  ...
  constructor(opts) {
    ...
    this.inputSchema = opts.inputSchema ? chunkVWJ7H3MJ_cjs.toStandardSchema(opts.inputSchema) : void 0;
    this.outputSchema = opts.outputSchema ? chunkVWJ7H3MJ_cjs.toStandardSchema(opts.outputSchema) : void 0;
    this.suspendSchema = opts.suspendSchema ? chunkVWJ7H3MJ_cjs.toStandardSchema(opts.suspendSchema) : void 0;
    this.resumeSchema = opts.resumeSchema ? chunkVWJ7H3MJ_cjs.toStandardSchema(opts.resumeSchema) : void 0;
        ...
        if (!context) {
          ...
          if (isAgentExecution && !baseContext.agent) {
            ...
            organizedContext = {
              ...
              requestContext: rest.requestContext || new chunk5WBEMKE2_cjs.RequestContext()
            };
            ...
            organizedContext = {
              ...
              requestContext: baseContext.requestContext || new chunk5WBEMKE2_cjs.RequestContext()
            };
          }
        }
...
//# sourceMappingURL=chunk-NQXA2BUF.cjs.map
//# sourceMappingURL=chunk-NQXA2BUF.cjs.map