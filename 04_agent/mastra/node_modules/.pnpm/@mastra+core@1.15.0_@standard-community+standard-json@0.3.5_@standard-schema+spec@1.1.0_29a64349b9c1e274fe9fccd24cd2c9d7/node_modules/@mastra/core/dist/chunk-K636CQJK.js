import { getOrCreateSpan, EntityType, executeWithContext, wrapMastra, createObservabilityContext } from './chunk-3PUUIEVX.js';
import { ToolStream } from './chunk-DD2VNRQM.js';
import { Tool, isVercelTool, validateToolInput, validateToolOutput, validateToolSuspendData } from './chunk-UPXPPD5L.js';
import { isZodObject } from './chunk-RT2LCNDN.js';
import { standardSchemaToJSONSchema, toStandardSchema, isStandardSchemaWithJSON } from './chunk-6VAVASZ5.js';
...
var CoreToolBuilder = class extends MastraBase {
    ...
    if (!isVercelTool(this.originalTool) && (input.autoResumeSuspendedTools || this.originalTool.id?.startsWith("agent-") || this.originalTool.id?.startsWith("workflow-"))) {
      ...
      if (isZodObject(schema)) {
        this.originalTool.inputSchema = schema.extend({
          suspendedToolRunId: z$1.string().describe("The runId of the suspended tool").nullable().optional(),
          resumeData: z$1.any().describe("The resumeData object created from the resumeSchema of suspended tool").optional()
        });
      } else {
  ...
  getParameters = () => {
    if (isVercelTool(this.originalTool)) {
      let schema2 = this.originalTool.parameters ?? ("inputSchema" in this.originalTool ? this.originalTool.inputSchema : void 0) ?? z$1.object({});
    ...
    if ("type" in tool && (tool.type === "provider-defined" || tool.type === "provider") && "id" in tool && typeof tool.id === "string" && tool.id.includes(".")) {
      ...
      if (parameters !== void 0 && parameters !== null) {
        if (typeof parameters === "object" && "jsonSchema" in parameters) {
          ...
          const jsonSchema2 = standardSchemaToJSONSchema(parameters, { io: "output" });
  ...
  build() {
    ...
    if (isStandardSchemaWithJSON(originalSchema)) {
      const inputJsonSchema = applicableLayer ? applicableLayer.toJSONSchema(originalSchema) : standardSchemaToJSONSchema(originalSchema, { io: "input" });
    ...
    return {
      ...
      parameters: processedInputSchema ?? z$1.object({}),
...
async function fetchWithRetry(url, options = {}, maxRetries = 3) {
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      const delay2 = Math.min(1e3 * Math.pow(2, retryCount), 1e4);
...
//# sourceMappingURL=chunk-K636CQJK.js.map
//# sourceMappingURL=chunk-K636CQJK.js.map