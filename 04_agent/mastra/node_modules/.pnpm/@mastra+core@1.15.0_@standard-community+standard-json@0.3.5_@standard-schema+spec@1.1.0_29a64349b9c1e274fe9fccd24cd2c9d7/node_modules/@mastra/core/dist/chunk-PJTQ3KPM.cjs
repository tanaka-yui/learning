...
var StepExecutor = class extends chunkRO47SMI7_cjs.MastraBase {
  ...
  async execute(params) {
    ...
    const { inputData, validationError } = await chunkBXSSIONK_cjs.validateStepInput({
    ...
    try {
      ...
      const stepOutput = await chunkJUL4UHWQ_cjs.executeWithContext({
        span: params.tracingContext?.currentSpan,
        fn: () => step.execute(
          chunkBXSSIONK_cjs.createDeprecationProxy(
            {
              ...
              getStepResult: chunkBXSSIONK_cjs.getStepResult.bind(this, stepResults),
              suspend: async (suspendPayload, suspendOptions) => {
                const { suspendData, validationError: validationError2 } = await chunkBXSSIONK_cjs.validateStepSuspendData({
              ...
              ...chunkJUL4UHWQ_cjs.createObservabilityContext(params.tracingContext)
            },
            {
              paramName: "runCount",
              deprecationMessage: chunkBXSSIONK_cjs.runCountDeprecationMessage,
              logger: this.logger
            }
          )
        )
      ...
      return {
        ...
        tripwire: error instanceof chunkBXSSIONK_cjs.TripWire ? {
      ...
      chunkBXSSIONK_cjs.createDeprecationProxy(
        {
          ...
          abortSignal: abortController?.signal,
          // TODO
          ...chunkJUL4UHWQ_cjs.createObservabilityContext(),
          iterationCount
        },
        {
          paramName: "runCount",
...
function createTripWireFromChunk(chunk) {
  const { payload } = chunk;
  return new chunkBXSSIONK_cjs.TripWire(
...
async function processWorkflowLoop({
  ...
  const reqContext = new chunk5WBEMKE2_cjs.RequestContext(Object.entries(requestContext ?? {}));
  ...
  if (resumeData !== void 0 && forEachIndex === void 0 && currentResult?.output?.length > 0) {
    ...
    if (suspendedIndices.length > 0) {
      ...
      for (const suspIdx of indicesToResume) {
        updatedOutput[suspIdx] = chunkVSGRSX3N_cjs.createPendingMarker();
      }
  ...
  async processWorkflowStepRun({
    ...
    if (step.step instanceof EventedWorkflow || step.step.component === "WORKFLOW") {
      if (resumeSteps?.length === 1 && resumeSteps[0] === step.step.id) {
        ...
        const timeTravelParams = chunkBXSSIONK_cjs.createTimeTravelExecutionParams({
    ...
    const rc = new chunk5WBEMKE2_cjs.RequestContext();
    for (const [key, value] of Object.entries(requestContext)) {
      rc.set(key, value);
    }
    const { resumeData: timeTravelResumeData, validationError: timeTravelResumeValidationError } = await chunkBXSSIONK_cjs.validateStepResumeData({
...
function cloneWorkflow(workflow, opts) {
  const wf = new chunkBXSSIONK_cjs.Workflow({
    id: opts.id,
...
function isAgent(input) {
  return input instanceof chunkBXSSIONK_cjs.Agent;
}
function isToolStep(input) {
  return input instanceof chunkNQXA2BUF_cjs.Tool;
}
function isStepParams(input) {
  return input !== null && typeof input === "object" && "id" in input && "execute" in input && !(input instanceof chunkBXSSIONK_cjs.Agent) && !(input instanceof chunkNQXA2BUF_cjs.Tool);
}
function isProcessor(obj) {
  return obj !== null && typeof obj === "object" && "id" in obj && typeof obj.id === "string" && !(obj instanceof chunkBXSSIONK_cjs.Agent) && !(obj instanceof chunkNQXA2BUF_cjs.Tool) && (typeof obj.processInput === "function" || typeof obj.processInputStep === "function" || typeof obj.processOutputStream === "function" || typeof obj.processOutputResult === "function" || typeof obj.processOutputStep === "function");
}
...
function createStepFromParams(params) {
  return {
    ...
    inputSchema: chunkVWJ7H3MJ_cjs.toStandardSchema(params.inputSchema),
    stateSchema: params.stateSchema ? chunkVWJ7H3MJ_cjs.toStandardSchema(params.stateSchema) : void 0,
    outputSchema: chunkVWJ7H3MJ_cjs.toStandardSchema(params.outputSchema),
...
async function processAgentStream(params) {
  ...
  for await (const chunk of fullStream) {
    ...
    if (streamFormat !== "legacy") {
      await chunkBXSSIONK_cjs.forwardAgentStreamChunk({ writer, chunk });
    }
  }
  try {
    await pubsub.publish(`workflow.events.v2.${runId}`, {
...
function createStepFromAgent(params, agentOrToolOptions) {
  ...
  return {
    ...
    execute: async ({
      ...
      const observabilityContext = chunkJUL4UHWQ_cjs.resolveObservabilityContext(obsFields);
      const logger = mastra?.getLogger();
      const toolData = {
        name: params.name,
        args: inputData
      ...
      const isV2Model = chunkBXSSIONK_cjs.isSupportedLanguageModel(modelInfo);
...
function createStepFromProcessor(processor) {
  ...
  return {
    ...
    inputSchema: chunkVWJ7H3MJ_cjs.toStandardSchema(chunkBXSSIONK_cjs.ProcessorStepSchema),
    outputSchema: chunkVWJ7H3MJ_cjs.toStandardSchema(chunkBXSSIONK_cjs.ProcessorStepOutputSchema),
    execute: async ({ inputData, requestContext, ...obsFields }) => {
      ...
      const abort = (reason, options) => {
        throw new chunkBXSSIONK_cjs.TripWire(reason || `Tripwire triggered by ${processor.id}`, options, processor.id);
      };
      ...
      if (processorStates) {
        ...
        if (!ps) {
          ps = new chunkBXSSIONK_cjs.ProcessorState();
      ...
      const executePhaseWithSpan = async (fn) => {
        try {
          ...
          if (error instanceof chunkBXSSIONK_cjs.TripWire) {
        ...
        switch (phase) {
          case "input": {
            if (processor.processInput) {
              ...
              if (result instanceof chunkO6O5MD7Q_cjs.MessageList) {
                ...
                chunkBXSSIONK_cjs.ProcessorRunner.applyMessagesToMessageList(
                  result,
          ...
          case "inputStep": {
            if (processor.processInputStep) {
              ...
              const validatedResult = await chunkBXSSIONK_cjs.ProcessorRunner.validateAndFormatProcessInputStepResult(result, {
                messageList: passThrough.messageList,
                processor,
                stepNumber: stepNumber ?? 0
              });
...
var EventedWorkflow = class extends chunkBXSSIONK_cjs.Workflow {
  constructor(params) {
    super(params);
...
var EventedRun = class extends chunkBXSSIONK_cjs.Run {
  constructor(params) {
    super(params);
  ...
  stream({
    ...
    this.streamOutput = new chunkBXSSIONK_cjs.WorkflowRunOutput({
      runId: this.runId,
      workflowId: this.workflowId,
      stream
    });
  ...
  async resume(params) {
    ...
    const requestContext = new chunk5WBEMKE2_cjs.RequestContext();
...
var EventedExecutionEngine = class extends chunkBXSSIONK_cjs.ExecutionEngine {
  eventProcessor;
  constructor({
    mastra,
    eventProcessor,
  ...
  async execute(params) {
    ...
    const finishCb = async (event, ack) => {
      ...
      if (["workflow.end", "workflow.fail", "workflow.suspend"].includes(event.type)) {
        ...
        if (event.type === "workflow.fail" && event.data.stepResults) {
          event.data.stepResults = chunkBXSSIONK_cjs.hydrateSerializedStepErrors(event.data.stepResults);
        }
    ...
    for (const [stepId, stepResult] of Object.entries(stepResultsWithoutTopLevelState)) {
      cleanStepResults[stepId] = chunkBXSSIONK_cjs.cleanStepResult(stepResult);
    }
    let callbackArg;
...
//# sourceMappingURL=chunk-PJTQ3KPM.cjs.map
//# sourceMappingURL=chunk-PJTQ3KPM.cjs.map