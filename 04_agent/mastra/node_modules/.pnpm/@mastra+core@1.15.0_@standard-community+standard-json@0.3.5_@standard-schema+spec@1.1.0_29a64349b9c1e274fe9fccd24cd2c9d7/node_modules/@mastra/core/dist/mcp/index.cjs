...
var MCPServerBase = class extends chunkRO47SMI7_cjs.MastraBase {
    ...
    if (this.agents && typeof this.agents === "object") {
      Object.entries(this.agents).forEach(([key, agent]) => {
        try {
          ...
          if (!(error instanceof chunk4U7ZLI36_cjs.MastraError) || error.id !== "MASTRA_ADD_AGENT_DUPLICATE_KEY") {
            throw error;
          }
        }
      });
    ...
    if (this.workflows && typeof this.workflows === "object") {
      Object.entries(this.workflows).forEach(([key, workflow]) => {
        try {
          ...
          if (!(error instanceof chunk4U7ZLI36_cjs.MastraError) || error.id !== "MASTRA_ADD_WORKFLOW_DUPLICATE_KEY") {
            throw error;
          }
        }
      });
   ...
   */
  constructor(config) {
    super({ component: chunk7XAECHYL_cjs.RegisteredLogger.MCP_SERVER, name: config.name });
