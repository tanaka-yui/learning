...
const BACKEND_URL = (import.meta.env.VITE_BACKEND_URL as string) || "http://localhost:4006";
const THEME_COLOR = (import.meta.env.VITE_THEME_COLOR as string) || "#10b981";
...
export default function App() {
  ...
  const send = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${BACKEND_URL}/agents/task-agent/text`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          input: userMsg,
          options: { memory: { userId: "user", conversationId: SESSION_ID } },
        }),
      });
