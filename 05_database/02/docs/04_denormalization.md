...
| 列名 | 日本語名 | 型 | 制約 | 備考 |
|------|---------|-----|------|------|
| post_id | 投稿ID | UUID | PK（複合）, FK → posts | |
| user_id | いいねしたユーザーID | UUID | PK（複合）, FK → users | |
| post_user_id | 投稿者ID | UUID | NOT NULL | posts.user_id のコピー（冗長） |
...
```sql
SELECT id, content, hashtags FROM posts WHERE user_id = :'my_id';
```
