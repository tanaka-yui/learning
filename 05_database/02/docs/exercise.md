...
- `\i /sql/02/exercise.sql` で演習用の出発点クエリを確認できること
...
- `posts` テーブルの `(user_id, created_at)` に複合インデックスを追加する
...
1. `04_denormalization.sql` の内容を参照して、以下が作成済みであることを確認する
   - `post_stats` テーブル（`like_count`, `reply_count`, `stamp_count`）
   - `posts.hashtags` JSON カラム
...
3. 最終版の `EXPLAIN ANALYZE` を実行して、Step 1 との実行時間を比較する
