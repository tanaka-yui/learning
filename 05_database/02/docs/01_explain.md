...
```
Seq Scan on posts  (cost=0.00..2831.00 rows=100 width=72) (actual time=0.024..18.342 rows=100 loops=1)
  Filter: (user_id = 42)
  Rows Removed by Filter: 99900
Planning Time: 0.123 ms
Execution Time: 18.401 ms
```
...
| 要素 | 説明 |
|-----|------|
| `cost=0.00..2831.00` | 推定コスト。`起動コスト..総コスト`（単位はページI/Oの相対コスト） |
| `rows=100` | オプティマイザの推定行数 |
| `width=72` | 1行あたりのバイト数（推定） |
| `actual time=0.024..18.342` | 実際の時間。`最初の行が出るまで(ms)..全行処理完了(ms)` |
| `rows=100`（actual側） | 実際に返した行数 |
| `loops=1` | このノードが実行されたループ回数（JOINの内側では1より大きくなる） |
| `Rows Removed by Filter: 99900` | フィルタ条件で除外された行数 |
...
SNS の `posts` テーブル（100,000件）で特定ユーザーの投稿を取得するクエリを例に、
...
```
Seq Scan on posts  (cost=0.00..2831.00 rows=100 width=72)
                   (actual time=0.024..18.342 rows=100 loops=1)
  Filter: (user_id = 42)
  Rows Removed by Filter: 99900
Execution Time: 18.5 ms
```

100,000件すべてを読み込み、99,900件を捨てています。
...
```
Index Scan using idx_posts_user_id on posts  (cost=0.29..152.40 rows=100 width=72)
...
|-----|--------------|--------------|-------|
| 総コスト（推定） | 2831.00 | 152.40 | 約 19 倍 |
| 実行時間 | 18.5 ms | 0.3 ms | 約 62 倍 |
| 読み込んだ行数 | 100,000 件 | 100 件（該当分のみ） | 1/1000 |

`idx_posts_user_id` インデックスによって、PostgreSQL は `user_id = 42` に該当する行の物理位置を直接特定できるようになります。
その結果、不要な 99,900 件の読み込みが完全に排除され、実行時間が大幅に短縮されます。
...
```sql
EXPLAIN (ANALYZE, BUFFERS) SELECT * FROM posts WHERE user_id = 42;
```
...
```sql
EXPLAIN (ANALYZE, FORMAT JSON) SELECT * FROM posts WHERE user_id = 42;
```
