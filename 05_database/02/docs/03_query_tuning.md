...
AND NOT EXISTS (
    ...
    WHERE m.user_id = :'my_id' AND m.mute_user_id = p.user_id
)
AND NOT EXISTS (
    -- 自分がミュートされた
    SELECT 1 FROM user_mutes m
    WHERE m.user_id = p.user_id AND m.mute_user_id = :'my_id'
);
```
...
SELECT p.*
FROM posts p
LEFT JOIN user_blocks b1 ON b1.user_id = :'my_id'      AND b1.block_user_id = p.user_id
LEFT JOIN user_blocks b2 ON b2.user_id = p.user_id AND b2.block_user_id = :'my_id'
...
**UUIDv7 を使ったキーセットページネーション**

UUIDv7 は時系列順なので、`id` カラムだけでページネーションができます：

```sql
-- 前ページ最後の id を使ったページネーション
SELECT * FROM posts
WHERE id < '01905a3b-7c10-7000-8000-000000000001'  -- 前ページ最後のID
ORDER BY id DESC
LIMIT 20;
```

`created_at` を使う場合と異なり、タイムスタンプのマイクロ秒衝突を気にする必要がありません。
