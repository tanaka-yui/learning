...
// CSRFトークンストア（セッションID -> CSRFトークン）
var (
	csrfTokens  = map[string]string{}
	csrfTokenMu sync.RWMutex
)
...
// パスワード変更ハンドラー: CSRFトークン検証あり（安全な実装）
...
func newServeMux() *http.ServeMux {
	...
	mux.HandleFunc("/csrf-token", handleCSRFToken)
...
func main() {
	mux := newServeMux()
	fmt.Println("CSRF対策版デモサーバーをポート8080で起動します")
