...
// SQLインジェクション攻撃: 文字列結合による脆弱性で全ユーザーが返却されることを検証する
func TestSQLInjectionAttack(t *testing.T) {
	handler := setupTestServer()
	...
	// 脆弱性により全ユーザー(3件)が返却されることを確認する
	if len(users) != 3 {
		t.Fatalf("SQLインジェクションにより全ユーザーが返却されるべき: got %d, want 3", len(users))
	}
}
