...
リクエストハンドラでは、**トークン発行を同期的に行い、SMTP 送信だけを非同期(ゴルーチン)**で実行する設計にしている。

```go
t := a.magic.Issue(u.Username)   // 同期: レスポンス前にトークンをストアへ保存
link := fmt.Sprintf(...)
go func() {
    // 非同期・ベストエフォート: SMTP が届かなくてもトークン発行は成立済み
    if err := sendMagicLinkMail(...); err != nil {
        log.Printf("...")
    }
}()
```

この設計により 2 つの問題を同時に解決している。

1. **ユーザ列挙(タイミングリーク)の排除** — ユーザが存在する場合も存在しない場合も、ハンドラは SMTP 完了を待たずに即座に応答する。応答時間の差でアカウントの有無を推測できない。
2. **ブラックホール SMTP によるリクエストブロックの排除** — `sendMagicLinkMail` は `net.Dialer{Timeout: 5s}` で接続タイムアウトを設けているが、そもそもゴルーチン内で実行されるためリクエスト処理をブロックしない。
