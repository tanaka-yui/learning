...
学習目的のため、本デモは次の点を意図的に省略している。本番実装では対処が必要である。

- **リフレッシュトークンはローテーションも期限管理もしない**: 発行したトークンは無期限に有効であり、再利用検出(reuse detection)も行わない。本番ではリフレッシュのたびに新しいトークンを発行し、使用済みトークンを無効化する「ローテーション + 再利用検出」が必要である(詳細は `docs/05_token_ops.md` を参照)。
- **リソースサーバは scope を強制しない**: `/api/me` はアクセストークンが有効であれば scope の値にかかわらずレスポンスを返す。本番ではトークンの scope に基づいてエンドポイントへのアクセス可否を判断する必要がある。また、Client Credentials グラントでは要求された scope をそのままトークンに記録しているが、AS が許可された scope だけに制限するべきである。
